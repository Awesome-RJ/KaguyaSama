...
def new_fed(bot: Bot, update: Update):
    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text("Failed to create federation! Head over to @InnocentBnda to notify us of the error.")
            return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
                message.reply_text("Failed to join federation! Please head to @InnocentBnda to report this.")
                return
